...
function inboxAppHtml() {
  ...
  function filtered(){const term=q("search").value.toLowerCase().trim();let rows=state.filter==="replies"?state.threads.filter(x=>x.unread_reply):state.threads;return term?rows.filter(x=>JSON.stringify(x).toLowerCase().includes(term)):rows}
  function render(){const rows=filtered();q("list").innerHTML=rows.length?rows.map(x=>'<div class="row '+(x.unread_reply?"reply ":"")+(x.phone===state.selected?"active":"")+'" data-phone="'+esc(x.phone)+'">'+(x.unread_reply?'<span class="replyDot" title="Unread customer reply"></span>':'')+'<div class="phone">'+esc(x.profile?.business_name||x.phone)+'</div><div style="color:#68717d;font-size:11px">'+esc(x.profile?.business_name?x.phone:"")+'</div><div class="preview">'+esc(x.latest?.text||"No preview")+'</div>'+(x.unread_reply?'<div class="replyLabel">● NEW REPLY</div>':'')+'<div class="time">'+esc(fmt(x.latest?.at))+'</div></div>').join(""):'<div class="empty">'+(state.filter==="replies"?"No replies yet.":"No SMS conversations yet.")+'</div>';document.querySelectorAll(".row").forEach(r=>r.onclick=()=>openThread(r.dataset.phone,{markRead:true}))}
  ...
  async function load({sync=false}={}){try{error("");q("refresh").disabled=true;q("refresh").textContent=sync?"Syncing…":"Refreshing…";if(sync){const synced=await api("/inbox/api/sync",{method:"POST",body:"{}"});toast("Synced "+(synced.restored_messages||0)+" sent messages · "+(synced.reply_threads||0)+" reply threads")}const j=await api("/inbox/api/threads");state.threads=j.threads||[];const replies=state.threads.filter(x=>x.unread_reply).length;q("replyCount").textContent=replies;q("inboxCount").textContent=state.threads.length+" conversations · "+replies+" unread repl"+(replies===1?"y":"ies");render();if(!state.selected&&state.threads.length&&window.innerWidth>720){await openThread(state.threads[0].phone,{markRead:false})}}catch(e){error(e.message)}finally{q("refresh").disabled=false;q("refresh").textContent="Refresh"}}
  async function openThread(phone,{markRead=true}={}){state.selected=phone;app.classList.add("open");let selected=state.threads.find(x=>x.phone===phone);q("threadPhone").innerHTML=esc(selected?.profile?.business_name||phone)+(selected?.unread_reply?' <span class="replyMeta">● New reply</span>':selected?.replied?' <span class="replyMeta" style="opacity:.65">● Replied</span>':'');q("messages").innerHTML='<div class="empty">Loading…</div>';try{const j=await api("/inbox/api/thread?phone="+encodeURIComponent(phone));q("messages").innerHTML=(j.messages||[]).map(m=>'<div class="bubble '+(m.direction==="outbound"?"out":"in")+'">'+linkify(m.text||"")+'<div class="meta">'+esc(fmt(m.at))+' · '+esc(m.status||"")+'</div></div>').join("")||'<div class="empty">No messages yet.</div>';q("messages").scrollTop=q("messages").scrollHeight;q("reply").disabled=false;q("send").disabled=false;if(markRead&&selected?.unread_reply){await api("/inbox/api/read",{method:"POST",body:JSON.stringify({phone})});selected.unread_reply=false;const unread=state.threads.filter(x=>x.unread_reply).length;q("replyCount").textContent=unread;q("inboxCount").textContent=state.threads.length+" conversations · "+unread+" unread repl"+(unread===1?"y":"ies");render();q("threadPhone").innerHTML=esc(selected?.profile?.business_name||phone)+(selected?.replied?' <span class="replyMeta" style="opacity:.65">● Replied</span>':'')}}catch(e){error(e.message)}}
