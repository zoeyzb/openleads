...
function inboxAppHtml() {
  ...
  state.drafts=loadDraftStore();updateDraftCount();let draftTimer=null;q("saveDraft").onclick=()=>saveDraft(state.selected,q("reply").value);q("reply").addEventListener("input",()=>{clearTimeout(draftTimer);draftTimer=setTimeout(()=>{if(state.selected)saveDraft(state.selected,q("reply").value,{quiet:true})},650)});
