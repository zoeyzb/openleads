...
async function listInboxThreads(limit = 100) {
  ...
  for (const phone of phones) {
    ...
    out.push({ phone, latest, profile, replied: replyPhones.has(phone) });
  }
  return out;
}
...
function inboxAppHtml() {
  ...
  function filtered(){const term=q("search").value.toLowerCase().trim();let rows=state.filter==="replies"?state.threads.filter(x=>x.replied):state.threads;return term?rows.filter(x=>JSON.stringify(x).toLowerCase().includes(term)):rows}
  function render(){const rows=filtered();q("list").innerHTML=rows.length?rows.map(x=>'<div class="row '+(x.replied?"reply ":"")+(x.phone===state.selected?"active":"")+'" data-phone="'+esc(x.phone)+'">'+(x.replied?'<span class="replyDot" title="Customer replied"></span>':'')+'<div class="phone">'+esc(x.profile?.business_name||x.phone)+'</div><div style="color:#68717d;font-size:11px">'+esc(x.profile?.business_name?x.phone:"")+'</div><div class="preview">'+esc(x.latest?.text||"No preview")+'</div>'+(x.replied?'<div class="replyLabel">● REPLIED</div>':'')+'<div class="time">'+esc(fmt(x.latest?.at))+'</div></div>').join(""):'<div class="empty">'+(state.filter==="replies"?"No replies yet.":"No SMS conversations yet.")+'</div>';document.querySelectorAll(".row").forEach(r=>r.onclick=()=>openThread(r.dataset.phone))}
  ...
  async function load({sync=false}={}){try{error("");q("refresh").disabled=true;q("refresh").textContent=sync?"Syncing…":"Refreshing…";if(sync){const synced=await api("/inbox/api/sync",{method:"POST",body:"{}"});toast("Synced "+(synced.restored_messages||0)+" sent messages · "+(synced.reply_threads||0)+" reply threads")}const j=await api("/inbox/api/threads");state.threads=j.threads||[];const replies=state.threads.filter(x=>x.replied).length;q("replyCount").textContent=replies;q("inboxCount").textContent=state.threads.length+" conversations · "+replies+" replied";render();if(!state.selected&&state.threads.length&&window.innerWidth>720){await openThread(state.threads[0].phone)}}catch(e){error(e.message)}finally{q("refresh").disabled=false;q("refresh").textContent="Refresh"}}
  async function openThread(phone){state.selected=phone;render();app.classList.add("open");const selected=state.threads.find(x=>x.phone===phone);q("threadPhone").innerHTML=esc(selected?.profile?.business_name||phone)+(selected?.replied?' <span class="replyMeta">● Replied</span>':'');q("messages").innerHTML='<div class="empty">Loading…</div>';try{const j=await api("/inbox/api/thread?phone="+encodeURIComponent(phone));q("messages").innerHTML=(j.messages||[]).map(m=>'<div class="bubble '+(m.direction==="outbound"?"out":"in")+'">'+linkify(m.text||"")+'<div class="meta">'+esc(fmt(m.at))+' · '+esc(m.status||"")+'</div></div>').join("")||'<div class="empty">No messages yet.</div>';q("messages").scrollTop=q("messages").scrollHeight;q("reply").disabled=false;q("send").disabled=false}catch(e){error(e.message)}}
