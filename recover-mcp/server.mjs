...
function inboxAppHtml() {
  ...
  const state={threads:[],selected:"",filter:"all",sse:null,lastInboundId:""};const q=id=>document.getElementById(id),app=q("app");
  ...
  if (requestUrl.pathname === "/inbox/status" && req.method === "GET") {
    void (async () => {
      ...
      for (const raw of allMessages || []) {
        try { if (JSON.parse(raw)?.direction === "inbound") inboundMessages++; } catch {}
      }
