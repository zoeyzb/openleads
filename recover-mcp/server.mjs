...
async function rebuildSmsInboxFromSheet({ limit = 2000 } = {}) {
  ...
  for (const row of rows || []) {
    ...
    if (isCarrierRegistrationError(codes)) {
      ...
      if (queued?.queued) { retry40010++; if (row.row) retryRows.push(row.row); }
    }
  ...
  if (retryRows.length) {
    await SMS_SHEET_BRIDGE.writeBasicRetryStatuses({
      spreadsheetId:SMS_BULK_SPREADSHEET_ID,
      tabName:SMS_BULK_TAB_NAME,
      rows:retryRows,
      status:"RETRY_40010"
    }).catch(error => console.error("SMS retry batch sheet writeback error", error?.message || error));
  }
  const replies = await rebuildInboxReplyIndex().catch(() => ({ reply_threads:0 }));
  return { configured:true, scanned, restored, failed_fetches:failedFetches, retry_40010:retry40010, retry_rows:retryRows.length, suppressed, ...replies };
}
...
async function listInboxThreads(limit = 100) {
  ...
  const out = [];
  const chunkSize = 100;
  for (let start = 0; start < phones.length; start += chunkSize) {
    const chunk = phones.slice(start, start + chunkSize);
    const rows = await Promise.all(chunk.map(async (phone) => {
      const [ids, rawProfile, latestInboundScoreRaw, readScoreRaw] = await Promise.all([
        redis.zRange(`recover:sms:inbox:thread:${phone}`, 0, 0, { REV: true }),
        redis.get(`recover:sms:inbox:contact:${phone}`).catch(() => null),
        redis.zScore("recover:sms:inbox:reply-threads", phone).catch(() => null),
        redis.hGet("recover:sms:inbox:read", phone).catch(() => null)
      ]);
      const latestRaw = ids?.[0] ? await redis.hGet(messagesHash, ids[0]).catch(() => null) : null;
      let latest = null;
      let profile = null;
      try { latest = latestRaw ? JSON.parse(latestRaw) : null; } catch {}
      try { profile = rawProfile ? JSON.parse(rawProfile) : null; } catch {}
      const latestInboundScore = Number(latestInboundScoreRaw || 0);
      const readScore = Number(readScoreRaw || 0);
      const replied = replyPhones.has(phone);
      return { phone, latest, profile, replied, unread_reply: replied && latestInboundScore > readScore, latest_inbound_at: latestInboundScore || null };
    }));
    out.push(...rows);
  }
  return out;
...
function inboxAppHtml() {
  ...
  const failureInfo=m=>{const payload=m?.raw?.data?.payload||m?.raw?.payload||m?.raw||{};const errors=Array.isArray(payload?.errors)?payload.errors:[];const first=errors[0]||{};const to=Array.isArray(payload?.to)?(payload.to[0]||{}):{};return{code:String(first?.code||to?.error_code||payload?.error_code||""),title:String(first?.title||""),detail:String(first?.detail||to?.error_message||payload?.error_message||"")}};
  const statusHtml=m=>{const st=String(m?.status||"").toLowerCase();const label=st||"";const failed=/fail|undeliver|reject|expire|error|blocked/.test(st);const cls=/deliver/.test(st)&&!failed?"delivered":failed?"failed":"";const info=failed?failureInfo(m):null;const reason=failed&&info&&(info.code||info.title||info.detail)?'<div class="messageFailureReason"><strong>'+(info.code?esc(info.code)+' · ':'')+esc(info.title||"Delivery failed")+'</strong>'+(info.detail?'<div>'+esc(info.detail)+'</div>':'')+(m?.id?'<button class="resendBtn" type="button" data-resend-id="'+esc(m.id)+'">Resend</button>':'')+'</div>':"";return(label?'<span class="statusBadge '+cls+'">'+esc(label)+'</span>':"")+reason};
  ...
  async function load({sync=false}={}){try{error("");if(sync){const synced=await api("/inbox/api/sync",{method:"POST",body:"{}"});toast("Synced "+(synced.restored_messages||0)+" outbound messages · "+(synced.reply_threads||0)+" reply threads")}const stats=await api("/inbox/status");const replied=Number(stats.reply_threads||0);const unread=Number(stats.unread_replies||0);q("replyCount").textContent=replied;q("unreadCount").textContent=unread;q("failedCount").textContent=(stats.failed_messages??0);state.bulkPaused=Boolean(stats.bulk_paused);state.sendingReady=Boolean(stats?.sending_route?.ok);state.sendingBlockReason=String(stats?.sending_route?.reason||"");q("inboxCount").textContent=(stats.outbound_messages??0)+" attempted · "+(stats.delivered_messages??0)+" delivered · "+Math.max(0,(stats.sent_messages??0)-(stats.delivered_messages??0))+" sent/in transit · "+(stats.failed_messages??0)+" failed · "+replied+" replies";if(state.bulkPaused||!state.sendingReady){q("liveState").classList.remove("live");q("liveState").classList.add("offline");q("liveLabel").textContent=state.sendingReady?"PAUSED":"BLOCKED";q("liveState").title=state.sendingReady?"Bulk SMS is paused":("Sender route blocked: "+(state.sendingBlockReason||"not ready"))}else{q("liveState").classList.remove("offline");q("liveState").classList.add("live");q("liveLabel").textContent="LIVE";q("liveState").title="Telnyx sender route verified"}const j=await api("/inbox/api/threads");state.threads=j.threads||[];render();if(!state.selected&&state.threads.length&&window.innerWidth>720){await openThread(state.threads[0].phone,{markRead:false})}}catch(e){error(e.message)}}
  async function openThread(phone,{markRead=true}={}){state.selected=phone;app.classList.add("open");let selected=state.threads.find(x=>x.phone===phone);q("threadPhone").innerHTML=esc(selected?.profile?.business_name||phone)+(selected?.unread_reply?' <span class="replyMeta">● New reply</span>':selected?.replied?' <span class="replyMeta" style="opacity:.65">● Replied</span>':'');q("threadSub").textContent=(selected?.profile?.business_name?phone+" · ":"")+"SMS conversation";q("messages").innerHTML='<div class="empty">Loading…</div>';try{const j=await api("/inbox/api/thread?phone="+encodeURIComponent(phone));q("messages").innerHTML=(j.messages||[]).map(m=>'<div class="bubble '+(m.direction==="outbound"?"out":"in")+'">'+linkify(m.text||"")+'<div class="meta">'+esc(fmt(m.at))+statusHtml(m)+'</div></div>').join("")||'<div class="empty">No messages yet.</div>';document.querySelectorAll("[data-resend-id]").forEach(btn=>btn.onclick=async e=>{e.stopPropagation();if(btn.disabled)return;btn.disabled=true;const original=btn.textContent;btn.textContent="Sending…";try{await api("/inbox/api/resend",{method:"POST",body:JSON.stringify({message_id:btn.dataset.resendId})});toast("Resent");await openThread(phone,{markRead:false});await load({sync:false})}catch(err){toast(err.message||"Resend failed")}finally{btn.disabled=false;btn.textContent=original}});q("messages").scrollTop=q("messages").scrollHeight;q("reply").disabled=!state.sendingReady;q("send").disabled=!state.sendingReady;if(!state.sendingReady){q("reply").placeholder="Sending blocked until Telnyx sender route is verified";}else{q("reply").placeholder="Write a reply…";}const savedDraft=draftFor(phone);if(savedDraft&&!q("reply").value.trim())q("reply").value=savedDraft;if(markRead&&selected?.unread_reply){await api("/inbox/api/read",{method:"POST",body:JSON.stringify({phone})});selected.unread_reply=false;const replied=state.threads.filter(x=>x.replied).length;const unread=state.threads.filter(x=>x.unread_reply).length;q("replyCount").textContent=replied;q("unreadCount").textContent=unread;const stats=await api("/inbox/status");q("inboxCount").textContent=(stats.outbound_messages??0)+" attempted · "+(stats.delivered_messages??0)+" delivered · "+Math.max(0,(stats.sent_messages??0)-(stats.delivered_messages??0))+" sent/in transit · "+(stats.submitted_messages??0)+" submitted · "+(stats.failed_messages??0)+" failed · "+(stats.reply_threads??0)+" replies";render();q("threadPhone").innerHTML=esc(selected?.profile?.business_name||phone)+(selected?.replied?' <span class="replyMeta" style="opacity:.65">● Replied</span>':'')}}catch(e){error(e.message)}}
  ...
  let inboxReloadTimer=null,inboxReloadRunning=false,inboxReloadPending=false;
  async function flushInboxReload(){if(inboxReloadRunning){inboxReloadPending=true;return}inboxReloadRunning=true;try{await load({sync:false})}finally{inboxReloadRunning=false;if(inboxReloadPending){inboxReloadPending=false;scheduleInboxReload(250)}}}
  function scheduleInboxReload(delay=400){clearTimeout(inboxReloadTimer);inboxReloadTimer=setTimeout(()=>{inboxReloadTimer=null;void flushInboxReload()},delay)}
  function connectLive(){if(state.sse)try{state.sse.close()}catch{};const es=new EventSource("/inbox/events");state.sse=es;es.onopen=()=>{if(!state.sendingReady){q("liveState").classList.remove("live");q("liveState").classList.add("offline");q("liveLabel").textContent="BLOCKED";q("liveState").title="Sender route blocked: "+(state.sendingBlockReason||"not verified")}else if(state.bulkPaused){q("liveState").classList.remove("live");q("liveState").classList.add("offline");q("liveLabel").textContent="PAUSED";q("liveState").title="Bulk SMS is paused"}else{q("liveState").classList.add("live");q("liveState").classList.remove("offline");q("liveLabel").textContent="LIVE";q("liveState").title="Telnyx sender route verified"}};es.onerror=()=>{q("liveState").classList.remove("live");q("liveState").classList.add("offline");q("liveLabel").textContent="OFFLINE";q("liveState").title="Messaging connection is offline"};es.onmessage=e=>{let ev;try{ev=JSON.parse(e.data)}catch{return}if(ev.type!=="message"&&ev.type!=="status")return;const wasSelected=state.selected===ev.phone;if(ev.type==="message"&&ev.direction==="inbound"&&ev.id!==state.lastInboundId){state.lastInboundId=ev.id;if(!wasSelected||document.hidden)notifyInbound(null)}scheduleInboxReload(ev.type==="message"&&ev.direction==="inbound"?100:500)}}
    ...
    .then(async result => {
      ...
      if (String(fromNumber.messaging_profile_id || "") !== String(profileId)) {
        ...
        console.log("Telnyx sending number attached to messaging profile", {
          phone_hint:configuredFrom ? "••••" + configuredFrom.slice(-4) : null,
          profile_id:profileId
        });
      } else {
        console.log("Telnyx sending number already attached to messaging profile", {
          phone_hint:configuredFrom ? "••••" + configuredFrom.slice(-4) : null,
          profile_id:profileId
        });
      }
...
setTimeout(() => {
  void (async () => {
    ...
    console.log("Recover inbox delivery summary", {
      ...delivery,
      reply_threads: replyThreads,
      inbound_messages: inboundMessages
    });
  })().catch(error => console.error("Recover inbox delivery summary error", error?.message || error));
}, 22000);
