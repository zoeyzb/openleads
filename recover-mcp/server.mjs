...
function inboxAppHtml() {
  ...
  function filtered(){const term=q("search").value.toLowerCase().trim();let rows=state.threads;if(state.filter==="replies")rows=state.threads.filter(x=>x.replied);if(state.filter==="unread")rows=state.threads.filter(x=>x.unread_reply);return term?rows.filter(x=>JSON.stringify(x).toLowerCase().includes(term)):rows}
  function render(){const rows=filtered();q("list").innerHTML=rows.length?rows.map(x=>'<div class="row '+(x.unread_reply?"reply ":"")+(x.phone===state.selected?"active":"")+'" data-phone="'+esc(x.phone)+'">'+(x.unread_reply?'<span class="replyDot" title="Unread customer reply"></span>':'')+'<div class="phone">'+esc(x.profile?.business_name||x.phone)+'</div><div style="color:#68717d;font-size:11px">'+esc(x.profile?.business_name?x.phone:"")+'</div><div class="preview">'+esc(x.latest?.text||"No preview")+'</div>'+(x.unread_reply?'<div class="replyLabel">● NEW REPLY</div>':x.replied?'<div class="replyLabel" style="opacity:.55">REPLIED</div>':'')+'<div class="time">'+esc(fmt(x.latest?.at))+'</div></div>').join(""):'<div class="empty">'+(state.filter==="replies"?"No replied conversations yet.":state.filter==="unread"?"No unread replies.":"No SMS conversations yet.")+'</div>';document.querySelectorAll(".row").forEach(r=>r.onclick=()=>openThread(r.dataset.phone,{markRead:true}))}
  ...
  q("composer").onsubmit=e=>{e.preventDefault();const text=q("reply").value.trim();if(!text||!state.selected)return;if(state.pendingSend){toast("Undo or wait for the pending message first");return}const pending={phone:state.selected,text,timer:null};state.pendingSend=pending;q("reply").value="";q("send").disabled=true;const wrap=document.createElement("div");wrap.className="bubble out pendingBubble";wrap.setAttribute("data-pending-send","1");wrap.innerHTML=linkify(text)+'<div class="meta">Sending in 10 seconds · Undo available</div>';q("messages").appendChild(wrap);q("messages").scrollTop=q("messages").scrollHeight;q("undoText").textContent="Sending in 10s…";q("undoBar").classList.add("show");let left=10;const countdown=setInterval(()=>{if(state.pendingSend!==pending){clearInterval(countdown);return}left--;q("undoText").textContent="Sending in "+Math.max(0,left)+"s…";if(left<=0)clearInterval(countdown)},1000);pending.timer=setTimeout(()=>commitPendingSend(pending),10000)};
  async function toggleUnavailable(){const panel=q("failurePanel");if(panel.classList.contains("show")&&panel.dataset.mode==="unavailable"){panel.classList.remove("show");return}panel.dataset.mode="unavailable";panel.innerHTML='<div class="empty" style="padding:8px">Loading unavailable numbers…</div>';panel.classList.add("show");try{const data=await api("/inbox/unavailable");panel.innerHTML='<div style="font-weight:850;margin-bottom:6px">'+esc(data.count||0)+' unavailable numbers · automatically skipped</div><div class="failureDetail" style="margin-bottom:8px">These numbers are marked Not Available in the sheet and suppressed from future sends.</div>'+((data.items||[]).length?(data.items||[]).map(r=>'<div class="failureItem"><div><div class="failureTitle">'+esc(r.business_name||"Unknown business")+' · '+esc(r.phone_masked||"")+'</div><div class="failureDetail">'+esc(r.reason||"")+(r.sheet_row?' · row '+esc(r.sheet_row):'')+'</div></div><div class="failureCount">40001</div></div>').join(""):'<div class="failureDetail">No unavailable numbers.</div>')}catch(e){panel.innerHTML='<div class="error">'+esc(e.message)+'</div>'}}
  async function toggleFailures(){const panel=q("failurePanel");if(panel.classList.contains("show")&&panel.dataset.mode==="failures"){panel.classList.remove("show");return}panel.dataset.mode="failures";panel.innerHTML='<div class="empty" style="padding:8px">Loading failure reasons…</div>';panel.classList.add("show");try{const data=await api("/inbox/failures");q("failedCount").textContent=data.failed_messages||0;const rows=data.reasons||[];panel.innerHTML='<div style="font-weight:850;margin-bottom:6px">'+esc(data.failed_messages||0)+' failed · '+esc(data.failure_rate_percent||0)+'% failure rate</div>'+(rows.length?rows.slice(0,12).map(r=>'<div class="failureItem"><div><div class="failureTitle">'+esc(r.title||"Unknown failure")+(r.code?' · '+esc(r.code):'')+'</div>'+(r.detail?'<div class="failureDetail">'+esc(r.detail)+'</div>':'')+'</div><div class="failureCount">'+esc(r.count)+'</div></div>').join(""):'<div class="failureDetail">No failure reasons stored yet.</div>')}catch(e){panel.innerHTML='<div class="error">'+esc(e.message)+'</div>'}}
  function updateNotifyButton(){const b=q("notify");if(!("Notification" in window)){b.style.display="none";return}const on=Notification.permission==="granted";b.textContent=on?"Notifications on":"Notify";b.classList.toggle("notifyOn",on)}
  async function enableNotifications(){if(!("Notification" in window))return toast("Notifications are not supported in this browser");const p=await Notification.requestPermission();updateNotifyButton();toast(p==="granted"?"Reply notifications enabled":"Notifications were not enabled")}
  function notifyInbound(thread){if(!thread||Notification.permission!=="granted")return;const title="New reply · "+(thread.profile?.business_name||thread.phone);const body=thread.latest?.text||"New SMS reply";try{new Notification(title,{body,tag:"recover-"+thread.phone,renotify:true})}catch{}}
  function connectLive(){if(state.sse)try{state.sse.close()}catch{};const es=new EventSource("/inbox/events");state.sse=es;es.onopen=()=>{q("liveState").innerHTML='<span class="livePulse"></span>LIVE'};es.onerror=()=>{q("liveState").textContent="RECONNECTING"};es.onmessage=async e=>{let ev;try{ev=JSON.parse(e.data)}catch{return}if(ev.type==="message"||ev.type==="status"){const wasSelected=state.selected===ev.phone;await load();const thread=state.threads.find(x=>x.phone===ev.phone);if(ev.type==="message"&&ev.direction==="inbound"&&ev.id!==state.lastInboundId){state.lastInboundId=ev.id;if(!wasSelected||document.hidden)notifyInbound(thread);if(wasSelected&&!document.hidden)await openThread(ev.phone,{markRead:true});else if(state.filter==="unread"||state.filter==="replies")render() } else if(wasSelected) await openThread(ev.phone,{markRead:false});}}}
  q("search").oninput=render;document.querySelectorAll(".filter").forEach(b=>b.onclick=()=>{state.filter=b.dataset.filter;document.querySelectorAll(".filter").forEach(x=>x.classList.toggle("active",x===b));render()});q("refresh").onclick=()=>load({sync:true});q("notify").onclick=enableNotifications;q("failuresBtn").onclick=toggleFailures;q("unavailableBtn").onclick=toggleUnavailable;q("undoSend").onclick=cancelPendingSend;q("back").onclick=()=>app.classList.remove("open");q("reply").addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();q("composer").requestSubmit()}});document.addEventListener("visibilitychange",()=>{if(!document.hidden&&state.selected)openThread(state.selected,{markRead:true})});updateNotifyButton();connectLive();setInterval(()=>load(),60000);load({sync:true});
  </script></body></html>`;
}
